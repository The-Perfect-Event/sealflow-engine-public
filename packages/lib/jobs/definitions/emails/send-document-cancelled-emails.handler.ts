...
export const run = async ({ payload, io }: { payload: TSendDocumentCancelledEmailsJobDefinition; io: JobRunIO }) => {
  ...
  // Send cancellation emails to recipients who have been sent the document or
  // viewed it. CC recipients are INCLUDED: they receive a "you have been
  // copied — you'll get the signed copy on completion" email at send time, so
  // staying silent on cancellation would leave them waiting for a signed copy
  // that never arrives. (CCs are created with sendStatus=SENT, so the existing
