import { DEFAULT_DIMENSIONS, KNOWN_SUBTYPES, PREFIX_MAP, TAG_DETECT_REGEX } from './constants.js';
import type { DecodedField, FieldDimensions, FieldSubtype } from './types.js';
 ...
 */
const DIMENSION_REGEX = /^dimension\(\s*width\s*=\s*([\d.]+)\s*mm\s*,\s*height\s*=\s*([\d.]+)\s*mm\s*\)$/i;
...
export function decodeTag(raw: string): DecodedField {
  ...
  if (!shell) {
    throw new TagDecodeError('not a tag: missing {{ }} delimiters');
  }

  const inner = shell[1]!;
  const esIdx = inner.indexOf('_es_');
  if (esIdx === -1) {
    throw new TagDecodeError('missing _es_ marker');
  }
  ...
  if (!headMatch) {
    throw new TagDecodeError(`malformed prefix segment: "${head.trim()}"`);
  }
  ...
  if (!tail.startsWith(':')) {
    throw new TagDecodeError('missing recipient (no ":" after _es_)');
  }
  const tokens = tail
    .slice(1)
    .split(':')
  ...
  if (recipient === '') {
    throw new TagDecodeError('missing recipient');
  }

  // Adobe Sign semantics: signature and initials fields are ALWAYS required
  // for their assigned signer — a document must never complete without them.
  // Other field types default to optional unless starred (`{{*Field_es_...}}`)
  // or carrying an explicit `:required` directive.
  let required = requiredFromStar || type === 'SIGNATURE' || type === 'INITIALS';
  let declaredDimensions: FieldDimensions | undefined;
  let explicitSubtype: FieldSubtype | undefined;

  for (const token of tokens) {
    if (token === '') {
      continue; // tolerate trailing/double colons
    }
